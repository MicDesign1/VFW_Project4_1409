...
NSString * VFW_Project4_1409$ModuleRequireFormat = @"(function(exports){"
...
@implementation VFW_Project4_1409Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_vfw_project4_1409);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create VFW_Project4_1409 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "VFW_Project4_1409" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_vfw_project4_1409 = [[VFW_Project4_1409Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_vfw_project4_1409];
	
	NSString *_vfw_project4_1409NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _vfw_project4_1409NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_vfw_project4_1409 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:VFW_Project4_1409$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.vfw_project4_1409.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.vfw_project4_1409.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
