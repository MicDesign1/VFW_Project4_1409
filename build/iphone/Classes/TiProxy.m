...
//Rare exceptions to indicate a bug in the _vfw_project4_1409 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_vfw_project4_1409 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_vfw_project4_1409]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.vfw_project4_1409.module"
